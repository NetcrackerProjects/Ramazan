...
public class GameObjectFactory {
    ...
    public Tank createTank(Vector leftTop, Vector rightBottom) {
        return new Tank(leftTop, rightBottom, tokenManager.getNewId());
    }

    public Bullet createBullet(Vector leftTop, Vector rightBottom) {
        return new Bullet(leftTop, rightBottom, 1, tokenManager.getNewId());
    }

    public Bonus createBonus(Vector leftTop, Vector rightBottom) {
        return new Bonus(leftTop, rightBottom, 5, tokenManager.getNewId());
    }
}
